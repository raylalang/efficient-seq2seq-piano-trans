...
from data.symbolic_music_tokenizer import (
    SymbolicMusicTokenizer,
    TokenOnset,
    ONSET_SEC_UP_SAMPLING,
)
import data.symbolic_music_tokenizer as Tokenizer
from data.beat_context import (
    compute_beats_full_audio,
    group_beats_into_bars_simple,
    select_context_bar_window,
    slice_context_seconds,
    bar_index_for_time,
)
...
class SingleWavDataset(Dataset):
    def __init__(
        ...
        )  # Don't use constand seed here!!! (Overfitting problems).
    ...
    def __load_cache(self):
        ...
        self.dataframe_midi = dataframe_midi.sort_values(by="onset_sec").reset_index(
            drop=True
        )
    ...
    def _ensure_beats(self):
        if self._beat_times_sec is None:
            ...
            self._tempo, self._beat_times_sec = tempo, beats
    ...
    def __getitem__(self, index, begin=None, end=None):
        ...
        audio = torch.tensor(
            self.audio_h5[self.audio_idx]["audio"][audio_begin:audio_end]
        )
        ...
        max_token_len = (
            self.config.data.max_token_length
        )  # * cp_tokenizer.compound_word_size
        # assert max_token_len < TOKEN_END
        decoder_targets = torch.ones([max_token_len], dtype=torch.long) * TOKEN_PAD
        decoder_targets_frame_index = torch.ones([max_token_len], dtype=torch.long) * (
            self.n_frames - 1
        )  # max frame index
        encoder_decoder_mask = torch.zeros(
            [max_token_len, self.n_frames], dtype=torch.long
        )
        seq_len = 0
        ...
        # Add EOS
        cp_eos = (
            torch.ones([sm_tokenizer.compound_word_size], dtype=sel_tokens.dtype)
            * sm_tokenizer.PAD
        )
        ...
        # MIDI Sequence
        df_midi = self.dataframe_midi[
            (self.dataframe_midi["onset_sec"] >= begin_sec)
            & (self.dataframe_midi["onset_sec"] <= end_sec)
        ].copy(deep=True)
        if len(df_midi) > 0:
            # make a offset for onset_sec
            df_midi["onset_sec"] = (
                df_midi["onset_sec"] - begin_sec
            )  # Fix offset when using clip for training.
            sel_cp_tokens, seconds = sm_tokenizer.tokenize_dataframe(
                df_midi, sequence_type="performance"
            )
        ...
        seconds.extend(
            [second_per_frame * self.n_frames] * sm_tokenizer.compound_word_size
        )  # Add EOS seconds
        ...
        for i, sec in enumerate(seconds[: seq_len - 1]):
            ...
            frame_index = np.clip(
                frame_index, 0, self.n_frames - 1
            )  # Clip to valid frame index
            decoder_targets_frame_index[i] = frame_index

        # Encoder Decoder Mask
        seconds = seconds[: seq_len - 1]
        if (
            hasattr(self.config.model, "encoder_decoder_slide_window_size")
            and self.config.model.encoder_decoder_slide_window_size > 0
        ):
            for i, sec in enumerate(seconds):
                if i % 3 == 0:
                    encoder_decoder_mask[i, :] = 1  # Set the mask to 1 for onset tokens
                    assert (
                        Tokenizer.TokenOnset.is_instance(sel_tokens[i])
                        or sel_tokens[i] == sm_tokenizer.EOS
                        or sel_tokens[i] == sm_tokenizer.PAD
                    ), "Onset token expected, but got %s" % (sel_tokens[i])
                    continue
                frame_index = int(np.round(sec / second_per_frame))
                frame_index = np.clip(
                    frame_index, 0, self.n_frames - 1
                ...
                begin_index = frame_index - (
                    frame_index % self.config.model.encoder_decoder_slide_window_size
                )
                end_index = (
                    begin_index + self.config.model.encoder_decoder_slide_window_size
                )
        ...
        audio_name = torch.nn.functional.pad(
            audio_name, pad=[0, 256 - audio_name.size()[0]], value=ord(" ")
        )

        midi_path = [ord(x) for x in self.midi_path][:1024]
        midi_path = torch.tensor(midi_path, dtype=torch.long)
        midi_path = torch.nn.functional.pad(
            midi_path, pad=[0, 1024 - midi_path.size()[0]], value=ord(" ")
        )

        dataset_name = [ord(x) for x in self.dataset_name][:32]
        dataset_name = torch.tensor(dataset_name, dtype=torch.long)
        dataset_name = torch.nn.functional.pad(
            dataset_name, pad=[0, 32 - dataset_name.size()[0]], value=ord(" ")
        )

        if getattr(self.config.model, "use_context", False):
            tempo, beat_times = self._ensure_beats()
            bpb = int(getattr(self.config.model, "beats_per_bar", 4))
            ptrs = group_beats_into_bars_simple(beat_times, beats_per_bar=bpb)

            # center bar = bar covering the segment start
            center_bar = bar_index_for_time(
                beat_times, ptrs, begin_sec, beats_per_bar=bpb
            )
            side = int(
                getattr(self.config.model, "context_bars", 4) // 2
            )  # ±2 bars default
            start_bar, end_bar = select_context_bar_window(ptrs, center_bar, side)
            ctx_begin_sec, ctx_end_sec = slice_context_seconds(
                beat_times, ptrs, start_bar, end_bar
            )
            ...
            context_audio = torch.tensor(
                self.audio_h5[self.audio_idx]["audio"][ctx_begin:ctx_end]
            )

            mask = (beat_times >= ctx_begin_sec) & (beat_times <= ctx_end_sec + 1e-6)
            context_beat_times = torch.tensor(
                beat_times[mask] - ctx_begin_sec, dtype=torch.float32
            )

            row["context_audio"] = context_audio
            row["context_beat_times"] = context_beat_times
    ...
    def __init__(
        ...
        for idx, path in subset_data:
            ...
            mid_path = os.path.join(
                root_dir,
                config.data.cache_dir_name,
                os.path.splitext(path)[0].replace("/", ">") + ".mid",
            )
    ...
    for idx in range(cp_n):
        ...
        if family == Tokenizer.FamilyType.NOTE.value:
            ...
            if Tokenizer.TokenBeat in sm_tokenizer.token_tpye_set:
                ...
                start
                + get_val(cp_word, Tokenizer.TokenDur)
                / Tokenizer.DUR_TATUM_UP_SAMPLING
                * sec_per_quarter
            )
            ...
            if Tokenizer.TokenTempo in sm_tokenizer.token_tpye_set:
                tempo = (
                    get_val(cp_word, Tokenizer.TokenTempo)
                    * Tokenizer.TEMPO_DOWN_SAMPLING
                )
...
def my_main(config: OmegaConf):
    ...
    for dataset_dir in config.data.dataset_dirs:
        ...
        datasets.append(
            Audio2Midi_Dataset(
                config, dataset_dir, dataset_index=0, subset="test", random_clip=True
            )
        )
