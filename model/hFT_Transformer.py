...
class Encoder_SPEC2MIDI(nn.Module):
    def __init__(self, n_margin, n_frame, n_bin, cnn_channel, cnn_kernel, hid_dim, n_layers, n_heads, pf_dim, dropout):
        super().__init__()
        ...
        self.layers_freq = nn.ModuleList([EncoderLayer(hid_dim, n_heads, pf_dim, dropout) for _ in range(n_layers)])
        self.dropout = nn.Dropout(dropout)

        scale_freq = torch.sqrt(torch.FloatTensor([hid_dim]))
        self.register_buffer("scale_freq", scale_freq)

    def forward(self, spec_in):
        """_summary_

        Args:
            spec_in (torch.tensor): shape [batch_size, n_bin, n_margin+n_frame+n_margin].

        Returns:
            torch.tensor: shape [batch_size, n_frame, n_bin, hid_dim].
        """
        ...
        # position coding
        pos_freq = torch.arange(0, self.n_bin).unsqueeze(0).repeat(batch_size*self.n_frame, 1).to(spec_in.device)
...
class Decoder_SPEC2MIDI(nn.Module):
    def __init__(self, n_frame, n_bin, n_note, n_velocity, hid_dim, n_layers, n_heads, pf_dim, dropout):
        super().__init__()
        self.n_note = n_note
        self.n_frame = n_frame
        ...
        self.layer_zero_freq = DecoderLayer_Zero(hid_dim, n_heads, pf_dim, dropout)
        self.layers_freq = nn.ModuleList([DecoderLayer(hid_dim, n_heads, pf_dim, dropout) for _ in range(n_layers-1)])
        ...
        # SAtime
        self.scale_time = torch.sqrt(torch.FloatTensor([hid_dim]))
        self.pos_embedding_time = nn.Embedding(n_frame, hid_dim)
        #self.layers_time = nn.ModuleList([DecoderLayer(hid_dim, n_heads, pf_dim, dropout) for _ in range(n_layers)])
        self.layers_time = nn.ModuleList([EncoderLayer(hid_dim, n_heads, pf_dim, dropout) for _ in range(n_layers)])
    ...
    def forward(self, enc_spec):
        ...
        ##
        pos_freq = torch.arange(0, self.n_note).unsqueeze(0).repeat(batch_size*self.n_frame, 1)
        ...
        pos_time = torch.arange(0, self.n_frame).unsqueeze(0).repeat(batch_size*self.n_note, 1)
